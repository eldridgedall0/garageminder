...
function openVehicleReportExportModal() {
  ...
  const modalHtml = '<div id="export-modal-overlay" class="export-modal-overlay"><div class="export-modal"><div class="export-modal-header"><h2><i class="bi bi-file-earmark-text"></i> Vehicle Report Export</h2><button type="button" class="export-modal-close" onclick="closeVehicleReportExportModal()">&times;</button></div><div class="export-modal-body"><div class="export-vehicle-info"><strong>' + escapeHtml(vehicle.name) + '</strong><span>' + entries.length + ' service record' + (entries.length !== 1 ? 's' : '') + '</span></div><div class="export-section"><label class="export-section-label">Export Format</label><div class="export-format-options"><label class="export-format-option ' + (savedPrefs.format === 'pdf' ? 'selected' : '') + '"><input type="radio" name="export-format" value="pdf" ' + (savedPrefs.format === 'pdf' ? 'checked' : '') + '><i class="bi bi-file-earmark-pdf"></i><span>PDF</span><small>Recommended</small></label><label class="export-format-option ' + (savedPrefs.format === 'word' ? 'selected' : '') + '"><input type="radio" name="export-format" value="word" ' + (savedPrefs.format === 'word' ? 'checked' : '') + '><i class="bi bi-file-earmark-word"></i><span>Word</span><small>.doc file</small></label><label class="export-format-option ' + (savedPrefs.format === 'xlsx' ? 'selected' : '') + '"><input type="radio" name="export-format" value="xlsx" ' + (savedPrefs.format === 'xlsx' ? 'checked' : '') + '><i class="bi bi-file-earmark-spreadsheet"></i><span>Excel</span><small>.xlsx file</small></label><label class="export-format-option ' + (savedPrefs.format === 'csv' ? 'selected' : '') + '"><input type="radio" name="export-format" value="csv" ' + (savedPrefs.format === 'csv' ? 'checked' : '') + '><i class="bi bi-filetype-csv"></i><span>CSV</span><small>Simple data</small></label></div></div><div class="export-section"><label class="export-section-label">Date Range</label><div class="export-date-options"><select id="export-date-range" class="export-select"><option value="all" ' + (savedPrefs.dateRange === 'all' ? 'selected' : '') + '>All Time</option><option value="year" ' + (savedPrefs.dateRange === 'year' ? 'selected' : '') + '>Last 12 Months</option><option value="6months" ' + (savedPrefs.dateRange === '6months' ? 'selected' : '') + '>Last 6 Months</option><option value="3months" ' + (savedPrefs.dateRange === '3months' ? 'selected' : '') + '>Last 3 Months</option><option value="ytd" ' + (savedPrefs.dateRange === 'ytd' ? 'selected' : '') + '>Year to Date</option><option value="custom" ' + (savedPrefs.dateRange === 'custom' ? 'selected' : '') + '>Custom Range</option></select><div id="export-custom-dates" class="export-custom-dates" style="display: ' + (savedPrefs.dateRange === 'custom' ? 'flex' : 'none') + ';"><input type="date" id="export-date-from" value="' + (savedPrefs.dateFrom || '') + '"><span>to</span><input type="date" id="export-date-to" value="' + (savedPrefs.dateTo || '') + '"></div></div></div><div class="export-section export-sections-group"><label class="export-section-label">Report Sections <small>(PDF/Word)</small></label><div class="export-checkbox-group"><label class="export-checkbox"><input type="checkbox" id="export-inc-vehicleinfo" ' + (savedPrefs.includeVehicleInfo !== false ? 'checked' : '') + '><span>Vehicle Information</span></label><label class="export-checkbox"><input type="checkbox" id="export-inc-renewals" ' + (savedPrefs.includeRenewals !== false ? 'checked' : '') + '><span>Renewal Dates</span></label><label class="export-checkbox"><input type="checkbox" id="export-inc-costsummary" ' + (savedPrefs.includeCostSummary === true ? 'checked' : '') + '><span>Cost Summary</span></label><label class="export-checkbox"><input type="checkbox" id="export-inc-reminders" ' + (savedPrefs.includeReminders !== false ? 'checked' : '') + '><span>Upcoming Maintenance</span></label><label class="export-checkbox"><input type="checkbox" id="export-inc-history" ' + (savedPrefs.includeHistory !== false ? 'checked' : '') + '><span>Service History</span></label><label class="export-checkbox"><input type="checkbox" id="export-inc-servicesummary" ' + (savedPrefs.includeServiceSummary === true ? 'checked' : '') + '><span>Service Type Summary</span></label></div></div><div class="export-section export-sections-group"><label class="export-section-label">Vehicle Details <small>(PDF/Word)</small></label><div class="export-checkbox-group"><label class="export-checkbox"><input type="checkbox" id="export-vd-vin" ' + (savedPrefs.showVin !== false ? 'checked' : '') + '><span>VIN</span></label><label class="export-checkbox"><input type="checkbox" id="export-vd-plate" ' + (savedPrefs.showPlate !== false ? 'checked' : '') + '><span>License Plate</span></label><label class="export-checkbox"><input type="checkbox" id="export-vd-odometer" ' + (savedPrefs.showOdometer !== false ? 'checked' : '') + '><span>Odometer</span></label><label class="export-checkbox"><input type="checkbox" id="export-vd-yearMakeModel" ' + (savedPrefs.showYearMakeModel !== false ? 'checked' : '') + '><span>Year/Make/Model</span></label><label class="export-checkbox"><input type="checkbox" id="export-vd-engine" ' + (savedPrefs.showEngine === true ? 'checked' : '') + '><span>Engine</span></label><label class="export-checkbox"><input type="checkbox" id="export-vd-stats" ' + (savedPrefs.showStats !== false ? 'checked' : '') + '><span>History Stats</span></label></div></div></div><div class="export-modal-footer"><button type="button" class="export-btn-secondary" onclick="closeVehicleReportExportModal()">Cancel</button><button type="button" class="export-btn-primary" onclick="executeVehicleReportExport()"><i class="bi bi-download"></i> Export Report</button></div></div></div>';
  ...
  document.querySelectorAll('input[name="export-format"]').forEach(function(radio) {
    radio.addEventListener('change', function() {
      ...
      document.querySelectorAll('.export-sections-group').forEach(function(group) {
        group.style.display = (radio.value === 'csv') ? 'none' : 'block';
      });
    });
  });
...
function loadExportPreferences() {
  try { var saved = localStorage.getItem('gm_export_prefs_v2'); if (saved) return JSON.parse(saved); } catch (e) {}
  return { format: 'pdf', dateRange: 'all', includeVehicleInfo: true, includeRenewals: true, includeCostSummary: false, includeReminders: true, includeHistory: true, includeServiceSummary: false, showVin: true, showPlate: true, showOdometer: true, showYearMakeModel: true, showEngine: false, showStats: true };
}
...
function getExportOptions() {
  ...
  return {
    ...
    includeVehicleInfo: document.getElementById('export-inc-vehicleinfo') ? document.getElementById('export-inc-vehicleinfo').checked : true,
    includeRenewals: document.getElementById('export-inc-renewals') ? document.getElementById('export-inc-renewals').checked : true,
    includeCostSummary: document.getElementById('export-inc-costsummary') ? document.getElementById('export-inc-costsummary').checked : false,
    includeReminders: document.getElementById('export-inc-reminders') ? document.getElementById('export-inc-reminders').checked : true,
    includeHistory: document.getElementById('export-inc-history') ? document.getElementById('export-inc-history').checked : true,
    includeServiceSummary: document.getElementById('export-inc-servicesummary') ? document.getElementById('export-inc-servicesummary').checked : false,
    showVin: document.getElementById('export-vd-vin') ? document.getElementById('export-vd-vin').checked : true,
    showPlate: document.getElementById('export-vd-plate') ? document.getElementById('export-vd-plate').checked : true,
    showOdometer: document.getElementById('export-vd-odometer') ? document.getElementById('export-vd-odometer').checked : true,
    showYearMakeModel: document.getElementById('export-vd-yearMakeModel') ? document.getElementById('export-vd-yearMakeModel').checked : true,
    showEngine: document.getElementById('export-vd-engine') ? document.getElementById('export-vd-engine').checked : false,
    showStats: document.getElementById('export-vd-stats') ? document.getElementById('export-vd-stats').checked : true
  };
}
...
function formatReminderDue(reminder, unit) {
  ...
  if (reminder.nextDate) {
    ...
    var options = sameYear ? { month: 'short', day: 'numeric' } : { month: 'short', day: 'numeric', year: 'numeric' };
...
function buildVehicleReportData(options) {
  ...
  var entries = data.entries.filter(function(e) { return e.vehicleId === activeVehicleId; }).sort(function(a, b) { return (a.date || '').localeCompare(b.date || ''); });
  ...
  var timeline = entries.map(function(e, index) {
    ...
    return {
      ...
      services: services, serviceNames: services.map(function(s) { return s.name; }),
      serviceCosts: services.filter(function(s) { return s.cost != null; }).map(function(s) { return s.name + ': $' + s.cost.toFixed(2); }).join('; '),
      totalCost: totalCost, totalCostFormatted: totalCost > 0 ? '$' + totalCost.toFixed(2) : '–', notes: e.notes || ''
    };
  });
  ...
  return {
    branding: branding, reportDate: todayIso, reportDateFormatted: formatDateNice(todayIso),
    reportTime: new Date().toLocaleTimeString(), unit: unit, unitFull: unit === 'km' ? 'Kilometers' : 'Miles',
    vehicle: {
    ...
    costBreakdown: costBreakdown, stats: stats, timeline: timeline, upcomingReminders: upcomingReminders, serviceSummary: serviceSummary, recordCount: timeline.length, options: options
  };
}
...
function calculateCostBreakdown(entries) {
  ...
  entries.forEach(function(entry) {
    if (Array.isArray(entry.services)) entry.services.forEach(function(s) { if (typeof s === 'object' && s.cost != null) partsCost += Number(s.cost) || 0; });
  ...
  return { parts: partsCost, labor: laborCost, misc: miscCost, total: total, partsFormatted: fmt(partsCost), laborFormatted: fmt(laborCost), miscFormatted: fmt(miscCost), totalFormatted: fmt(total) };
}
...
function calculateEnhancedStats(vehicle, entries, reminders) {
  ...
  var totalCost = 0, ytdCost = 0, firstDate = null, lastDate = null, firstOdo = null, lastOdo = null, serviceCount = 0;
  ...
  entries.forEach(function(e) {
    ...
    normalizeServices(e.services || []).forEach(function(s) { serviceTypeCounts[s.name] = (serviceTypeCounts[s.name] || 0) + 1; });
  });
  ...
  return {
    totalCost: totalCost, ytdCost: ytdCost, avgCostPerService: serviceCount > 0 ? totalCost / serviceCount : 0,
    costPerMile: distanceTracked > 0 ? totalCost / distanceTracked : 0,
    firstDate: firstDate, firstDateFormatted: firstDate ? formatDateNice(firstDate) : null,
    lastDate: lastDate, lastDateFormatted: lastDate ? formatDateNice(lastDate) : null,
    firstOdo: firstOdo, lastOdo: lastOdo, distanceTracked: distanceTracked, yearsTracked: yearsTracked, monthsTracked: monthsTracked,
    avgPerYear: yearsTracked > 0 ? Math.round(distanceTracked / yearsTracked) : distanceTracked,
    serviceCount: serviceCount, overdueCount: overdueCount, upcomingCount: upcomingCount, totalReminders: reminders.length,
...
function exportVehicleReportCSV(options) {
  ...
  lines.push('Date,Odometer (' + unit + '),Services,Service Costs,Total Cost,Notes');
  rpt.timeline.slice().reverse().forEach(function(entry) {
    lines.push(entry.dateFormatted + ',' + (entry.odometer != null ? entry.odometer : '') + ',"' + entry.serviceNames.join('; ').replace(/"/g, '""') + '","' + entry.serviceCosts.replace(/"/g, '""') + '",' + (entry.totalCost > 0 ? entry.totalCost.toFixed(2) : '') + ',"' + (entry.notes || '').replace(/"/g, '""') + '"');
  });
  lines.push('');
...
async function exportVehicleReportPDF(options) {
  ...
  var logoData = null;
  try { logoData = await loadImageAsDataUrl(branding.logoUrl); } catch (e) { console.warn('Could not load logo:', e); }
...
function generateCleanPDF(rpt, safeName, logoData) {
  ...
  var branding = rpt.branding, unit = rpt.unit, v = rpt.vehicle, s = rpt.stats, cost = rpt.costBreakdown, opts = rpt.options;
  
  var black = [0, 0, 0], darkGray = [51, 51, 51], mediumGray = [102, 102, 102];
  var lightGray = [153, 153, 153], veryLightGray = [220, 220, 220], tableBg = [245, 245, 245], white = [255, 255, 255];
  
  var y = 12;
  
  if (logoData) { try { doc.addImage(logoData, 'PNG', margin, y - 3, 9, 9); } catch (e) {} }
  var logoOffset = logoData ? 12 : 0;
  doc.setFontSize(18); doc.setFont('helvetica', 'bold'); doc.setTextColor(black[0], black[1], black[2]);
  doc.text(branding.appName, margin + logoOffset, y + 2);
  doc.setFontSize(9); doc.setFont('helvetica', 'normal'); doc.setTextColor(mediumGray[0], mediumGray[1], mediumGray[2]);
  doc.text('Vehicle Maintenance Report', margin + logoOffset, y + 7);
  doc.setFontSize(8); doc.setTextColor(darkGray[0], darkGray[1], darkGray[2]);
  ...
  y = 24;
  doc.setDrawColor(veryLightGray[0], veryLightGray[1], veryLightGray[2]); doc.setLineWidth(0.5);
  doc.line(margin, y, pageWidth - margin, y);
  y = 30;
  
  if (opts.includeVehicleInfo) {
    doc.setFillColor(darkGray[0], darkGray[1], darkGray[2]); doc.rect(margin, y, contentWidth, 7, 'F');
    ...
    var boxStartY = y;
    doc.setDrawColor(veryLightGray[0], veryLightGray[1], veryLightGray[2]); doc.setLineWidth(0.3);
    y += 6;
    doc.setFontSize(14); doc.setFont('helvetica', 'bold'); doc.setTextColor(black[0], black[1], black[2]);
    ...
    if (opts.showStats) {
      if (s.monthsTracked > 0) {
        var span = s.yearsTracked > 0 ? s.yearsTracked + ' year' + (s.yearsTracked > 1 ? 's' : '') : s.monthsTracked + ' month' + (s.monthsTracked > 1 ? 's' : '');
    ...
    detailItems.forEach(function(item) {
      ...
      var displayValue = item.value.length > 24 ? item.value.substring(0, 24) + '...' : item.value;
  ...
  if (opts.includeRenewals && (v.insuranceExpiry || v.registrationExpiry)) {
    ...
    y += 10;
    doc.setDrawColor(veryLightGray[0], veryLightGray[1], veryLightGray[2]); doc.rect(margin, boxStartY2, contentWidth, y - boxStartY2, 'S');
    y += 6;
  }
  ...
  if (opts.includeCostSummary && cost.total > 0) {
    ...
    costItems.forEach(function(item, i) {
      ...
      if (item.bold) { doc.setFillColor(tableBg[0], tableBg[1], tableBg[2]); doc.rect(x, y, cardWidth, 14, 'FD'); }
      else { doc.rect(x, y, cardWidth, 14, 'S'); }
      doc.setFontSize(6); doc.setTextColor(lightGray[0], lightGray[1], lightGray[2]); doc.setFont('helvetica', 'normal');
      doc.text(item.label.toUpperCase(), x + cardWidth / 2, y + 4, { align: 'center' });
      doc.setFontSize(item.bold ? 11 : 9); doc.setTextColor(item.bold ? black[0] : darkGray[0], item.bold ? black[1] : darkGray[1], item.bold ? black[2] : darkGray[2]);
      doc.setFont('helvetica', item.bold ? 'bold' : 'normal');
      doc.text(item.value, x + cardWidth / 2, y + 10, { align: 'center' });
    });
    y += 18;
    doc.setDrawColor(veryLightGray[0], veryLightGray[1], veryLightGray[2]); doc.rect(margin, boxStartY3, contentWidth, y - boxStartY3 + 2, 'S');
    y += 8;
  }
  ...
  if (opts.includeReminders && rpt.upcomingReminders.length > 0) {
    ...
    doc.setFillColor(s.overdueCount > 0 ? mediumGray[0] : lightGray[0], s.overdueCount > 0 ? mediumGray[1] : lightGray[1], s.overdueCount > 0 ? mediumGray[2] : lightGray[2]);
    ...
    var boxStartY4 = y;
    y += 4;
    rpt.upcomingReminders.forEach(function(reminder, i) {
      var rowY = y + (i * 6);
      var dotFill = reminder.status === 'overdue' ? black : (reminder.status === 'upcoming' ? mediumGray : lightGray);
      doc.setFillColor(dotFill[0], dotFill[1], dotFill[2]); doc.circle(margin + 5, rowY + 1.5, 1.5, 'F');
    ...
    doc.setDrawColor(veryLightGray[0], veryLightGray[1], veryLightGray[2]); doc.rect(margin, boxStartY4, contentWidth, y - boxStartY4, 'S');
    y += 6;
  }
  ...
  if (opts.includeHistory && rpt.timeline.length > 0) {
    ...
    var headers = ['Date', 'Odometer', 'Services', 'Cost', 'Notes'];
    var rows = rpt.timeline.slice().reverse().map(function(entry) {
      return [entry.dateFormatted, entry.odometerFormatted, entry.serviceNames.join(', ') || '–', entry.totalCostFormatted, entry.notes ? (entry.notes.length > 35 ? entry.notes.substring(0, 35) + '...' : entry.notes) : '–'];
    });
    ...
    doc.autoTable({
      ...
      didDrawPage: function(data) {
        doc.setFontSize(7); doc.setTextColor(mediumGray[0], mediumGray[1], mediumGray[2]);
...
function exportVehicleReportWord(options) {
  ...
  var html = '<!DOCTYPE html><html><head><meta charset="utf-8"><title>' + escapeHtml(branding.appName) + ' - Vehicle Report</title><style>@page{margin:0.75in;size:letter}*{box-sizing:border-box}body{font-family:Arial,sans-serif;font-size:10pt;line-height:1.4;color:#333;margin:0;padding:0}.header{border-bottom:1px solid #ccc;padding-bottom:12px;margin-bottom:20px}.header-row{display:table;width:100%}.header-left{display:table-cell;vertical-align:top}.header-right{display:table-cell;vertical-align:top;text-align:right}.app-name{font-size:18pt;font-weight:bold;color:#000}.app-tagline{font-size:9pt;color:#666;margin-top:2px}.report-meta{font-size:8pt;color:#333}.section{margin:15px 0;page-break-inside:avoid}.section-header{background:#333;color:#fff;padding:6px 12px;font-size:10pt;font-weight:bold}.section-body{border:1px solid #dcdcdc;border-top:none;padding:15px}.vehicle-name{font-size:14pt;font-weight:bold;color:#000;margin-bottom:12px}.info-grid{display:table;width:100%}.info-row{display:table-row}.info-cell{display:table-cell;width:33%;padding:5px 10px 5px 0;vertical-align:top}.info-label{font-size:7pt;color:#999;text-transform:uppercase}.info-value{font-size:9pt;color:#333;font-weight:bold}.reminder-row{padding:4px 0;border-bottom:1px solid #eee;display:table;width:100%}.reminder-row:last-child{border-bottom:none}.reminder-dot{display:table-cell;width:20px;vertical-align:middle}.reminder-dot-inner{width:8px;height:8px;border-radius:50%;display:inline-block}.reminder-dot-inner.overdue{background:#000}.reminder-dot-inner.upcoming{background:#666}.reminder-dot-inner.ok{background:#ccc}.reminder-name{display:table-cell;font-size:9pt;color:#333;vertical-align:middle}.reminder-due{display:table-cell;text-align:right;font-size:8pt;color:#666;vertical-align:middle}table.data-table{width:100%;border-collapse:collapse;font-size:9pt}table.data-table th{background:#f5f5f5;padding:6px 8px;text-align:left;font-size:8pt;text-transform:uppercase;color:#666;border-bottom:1px solid #ccc;font-weight:bold}table.data-table td{padding:6px 8px;border-bottom:1px solid #eee;vertical-align:top;color:#333}table.data-table tr:nth-child(even) td{background:#fafafa}.cost-cell{font-weight:bold;text-align:right}.footer{margin-top:20px;padding:12px;border:1px solid #dcdcdc;font-size:8pt;color:#666;line-height:1.5}</style></head><body>';

  html += '<div class="header"><div class="header-row"><div class="header-left"><div class="app-name">' + escapeHtml(branding.appName) + '</div><div class="app-tagline">Vehicle Maintenance Report</div></div><div class="header-right"><div class="report-meta">Report Date: ' + rpt.reportDateFormatted + '<br>' + rpt.recordCount + ' Service Record' + (rpt.recordCount !== 1 ? 's' : '') + '</div></div></div></div>';

  if (opts.includeVehicleInfo) {
    html += '<div class="section"><div class="section-header">Vehicle Information</div><div class="section-body"><div class="vehicle-name">' + escapeHtml(v.name) + '</div><div class="info-grid">';
    var detailItems = [];
    if (opts.showVin && v.vin !== 'Not Recorded') detailItems.push({ label: 'VIN', value: v.vin });
    if (opts.showPlate && v.plate !== 'Not Recorded') detailItems.push({ label: 'License Plate', value: v.plate });
    if (opts.showOdometer && v.currentOdo != null) detailItems.push({ label: 'Current Odometer', value: v.currentOdo.toLocaleString() + ' ' + unit });
  ...
  if (opts.includeReminders && rpt.upcomingReminders.length > 0) {
    ...
    rpt.upcomingReminders.forEach(function(reminder) {
      html += '<div class="reminder-row"><div class="reminder-dot"><span class="reminder-dot-inner ' + reminder.status + '"></span></div><div class="reminder-name">' + escapeHtml(reminder.serviceName) + '</div><div class="reminder-due">' + formatReminderDue(reminder, unit) + '</div></div>';
    });
    html += '</div></div>';
  }

  if (opts.includeHistory && rpt.timeline.length > 0) {
    html += '<div class="section"><div class="section-header">Service History</div><div class="section-body" style="padding:0;"><table class="data-table"><thead><tr><th>Date</th><th>Odometer</th><th>Services</th><th style="text-align:right;">Cost</th><th>Notes</th></tr></thead><tbody>';
    rpt.timeline.slice().reverse().forEach(function(entry) {
  ...
  html += '<div class="footer">This report is generated from records in ' + escapeHtml(branding.appName) + '. It may not include all maintenance performed on this vehicle. For vehicle purchase decisions, always verify records and perform an independent inspection.<br><em>Report generated: ' + rpt.reportDateFormatted + ' at ' + rpt.reportTime + '</em></div></body></html>';
